...
class myplots:
    ...
    def ap_plot(self, size=False):
        if size:
            fig = plt.figure(1, figsize=(size[0], size[1]))
        average_precision = average_precision_score(self.y_true, self.y_pred)
    ...
    def cal_plot(self, nbins, y_fixed=False, size=False):
        fraction_of_positives, mean_predicted_value = calibration_curve(self.y_true, self.y_pred, False, n_bins=nbins)
        ...
        ax2.hist(self.y_pred, range=(0, 1), bins=nbins, histtype="step", lw=2)
